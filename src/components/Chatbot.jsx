...
export default function HealthChatbotSection() {
  ...
  return (
    <>
      ...
      {/* Popup Chat Window */}
      {open && (
        <div className="fixed bottom-20 right-6 w-[350px] sm:w-[400px] h-[550px] bg-white/90 dark:bg-[#232946]/95 shadow-2xl rounded-3xl overflow-hidden flex flex-col border border-[#2563eb]/40 dark:border-[#60a5fa]/40 z-50 backdrop-blur-xl animate-fade-in">
          {/* Header */}
          <div className="flex justify-between items-center px-4 py-3 bg-gradient-to-r from-[#2563eb] to-[#60a5fa] text-white font-bold">
            <span className="flex items-center gap-2">
              <Bot className="w-5 h-5" /> Saksham – Health Assistant
            </span>
            <button
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {msg.actions.map((action, index) => (
                        <button
                          ...
                          className="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-[#2563eb] to-[#60a5fa] text-white shadow hover:scale-105 transition"
                        >
                          {action.label}
                        </button>
                      ))}
          ...
          {/* Input */}
          <div className="flex items-center gap-2 p-3 border-t border-[#2563eb]/20 dark:border-[#60a5fa]/20">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={handleKeyDown}
              placeholder="Type your message..."
              className="flex-1 bg-white/90 dark:bg-[#232946]/80 text-sm text-[#232946] dark:text-[#e0e7ef] rounded-full px-4 py-2 outline-none focus:ring-2 focus:ring-[#2563eb]"
            />
            <button
              onClick={handleSend}
              disabled={loading}
